...
export function stackedBar(svg, wordsPerGenre, search) {
  ...
  svg
    ...
    .on("mouseout", function () {
      d3.select(this).transition("colorfade").duration(250).attr("fill", color);
    })
    .attr("x", 50) //x(0) der kann so bleiben
    .attr("y", function (d, i) {
      return i * barHeight + 10;
    }) //.attr("y", (d,i) => (i * barHeight) +10)  //müsste je nach key runtergeschoben werden
    .attr("width", 0)
      ...
      for (var i = 0; i <= array.length - 1; i++) {
        if (array[i][0] == search) {
          return d3.descending(
            wordsPerGenre.get(a)[i][1],
            wordsPerGenre.get(b)[i][1]
          );
        }
      }
    });
  ...
  if (container.every((item) => item === 0)) {
    alert(
      search + " does not exist in any election program, try another word!"
    );
  }
  ...
  // Y axis
  var yRange = d3.scaleLinear().range([0, genre.length * 25]);
